...
void ledControl(int pin, int count); // 매개 변수로 제어 핀과 count 횟수를 가진
                                     // ledControl() 함수 선언
void motor_Control(byte dcStep); // DC 모터 동작과 관련된 motor_Control 함수
                                 // 선언
...
void loop() {
                            ...
                            // 입력되었을 때
  {
    ...
    case 1:
      ...
      ledControl(
          LIVINGROOM_LED,
          LivingroomLedCount); // 거실 LED 제어 핀과 LivingroomLedCount 값을
                               // 매개 변수로하여 ledControl()함수 호출
      break;
    case 2:
      roomLedCount++; // 2가 발생될 때마다 roomLedCount 변수 1증가
      ledControl(ROOM_LED,
                 roomLedCount); // 방 LED 제어 핀과 roomLedCount 값을 매개
                                // 변수로하여 ledControl()함수 호출
      break;
    case 3:
      motor_Control(motorStep); // 모터 속도 단계를 매개 변수로하여
                                // motor_Contorl() 함수 호출
      break;
    }
  }
...
void ledControl(int pin, int count) // ledControl() 함수 정의
{
  ...
  {
    digitalWrite(pin, HIGH); // LED를 제어하는 핀에 HIGH 신호를 발생시켜 LED
                             // 점등
  } else // count 값이 짝수일 때
  {
